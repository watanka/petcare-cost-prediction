...
class DataPreprocessPipeline(BasePreprocessPipeline):
    ...
    def define_pipeline(self):
        ...
        self.numerical_pipeline = Pipeline(
            [
                ("simple_imputer", SimpleImputer(missing_values=np.nan, strategy="constant", fill_value=None, add_indicator=True)),
                ("scaler", MinMaxScaler()),
            ]
        )
    ...
    def set_categories(self):
        ...
        # self.petbreed_categories = [
        #     f"one_hot_encoder__pet_breed_id_{c}" for c in self.pipeline.named_transformers_["categorical"].steps[-1][-1].categories_[0].tolist()
        # ]
        self.gender_categories = [
            "categorical__gender_남자","categorical__gender_여자"
        ...
        self.neutralized_categories = [
            "categorical__neuter_yn_y", "categorical__neuter_yn_n"   
    ...
    def preprocess(self, x) -> pd.DataFrame:
        ...
        return pd.DataFrame(self.pipeline.transform(x), columns = self.pipeline.get_feature_names_out())
    ...
    def inverse_transform(self, df):
        print(df)
        df['gender'] = df[self.gender_categories   ].idxmax(axis=1).apply(lambda x: x.split('_')[-1])
        df['neuter_yn'] = df[self.neutralized_categories].idxmax(axis=1).apply(lambda x: x.split('_')[-1])
        df['pet_breed_id'] = df[self.pet_breed_categories].idxmax(axis=1).apply(lambda x: x.split('_')[-1])
        
        print(self.pipeline.get_params()['transformers'][1][1])
        
        # numerical 역변환
        weightScaler = MinMaxScalerReverse(minval = self.weight_min, maxval = self.weight_max)
        ageScaler = MinMaxScalerReverse(minval = self.age_min, maxval = self.age_max)
        df['weight_kg'] = df['min_max_scaler__weight_kg'].apply(lambda x: weightScaler.inverse_transform(x))
        df['age'] = df['min_max_scaler__age'].apply(lambda x: ageScaler.inverse_transform(x))
        ...
        df.drop(['min_max_scaler__weight_kg','min_max_scaler__age'], inplace = True, axis = 1)
